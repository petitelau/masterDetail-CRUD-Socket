import {VALUE} from "./presentationModel.js";

export {Observable, ObservableList}

const Observable = value => {
    const listeners = [];

    const remove = (array) => (callback) => {
        const index = array.indexOf(callback);
        if (index >= 0) array.splice(index, 1);
    };

    const listenersRemove = remove(listeners);
    return {
        onChange: callback => {
            listeners.push(callback);
            callback(value, value);
            return () => listenersRemove(callback);
        },
        getValue: ()       => value,
        setValue: newValue => {
            if (value === newValue) return;
            const oldValue = value;
            value = newValue;
            listeners.forEach(callback => callback(value, oldValue));
        }
    }
};


const ObservableList = list => {
    const addListeners = [];
    const delListeners = [];
    const removeAt     = array => index => array.splice(index, 1);
    const removeItem   = array => item  => { const i = array.indexOf(item); if (i>=0) removeAt(array)(i); };
    const listRemoveItem     = removeItem(list);
    const delListenersRemove = removeAt(delListeners);

    return {
        onAdd: listener => addListeners.push(listener),
        onDel: listener => delListeners.push(listener),
        add: item => {
            list.push(item);
            addListeners.forEach( listener => listener(item))
            return item;
        },
        del: item => {
            listRemoveItem(item);
            const safeIterate = [...delListeners]; // shallow copy as we might change listeners array while iterating
            safeIterate.forEach( (listener, index) => listener(item, () => delListenersRemove(index) ));
        },
        delAll : ()=> {
            const safeIterate = [...list];
            safeIterate.forEach(item => {
                listRemoveItem(item);
                const safeIterate = [...delListeners]; //call with true at the end for only refresh view and not persist
                safeIterate.forEach( (listener, index) => listener(item, () => delListenersRemove(index) , true));
            })
        },

       // removeDeleteListener: removeItem(delListeners),
        count:   ()   => list.length,
        countIf: pred => list.reduce( (sum, item) => pred(item) ? sum + 1 : sum, 0),
        clear: ()=>list.length=0,
        print: ()=>  list.forEach(r=>console.log(r.name.getObs(VALUE).getValue())),
        compare: (objs) =>  list.every(r=> objs.includes(r.name.getObs(VALUE).getValue() ))
    }
};
